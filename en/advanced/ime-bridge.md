...
## Pro: Learn hotwords from corrections <Badge type="warning" text="Pro" />
...
1. Enable `Learn hotwords from corrections` under `Pro Settings → ASR Settings → Result Optimization (Pro)`
...
Password and other sensitive fields, plus editors that disallow personalized learning, do not expose context or correction observations. See [Pro Features](/en/pro/features#learn-hotwords-from-corrections).
