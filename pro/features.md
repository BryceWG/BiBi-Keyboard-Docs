...
- **识别后近音替换**: 基于音素匹配把近似发音替换为目标热词
- **从纠正中学习热词**: 根据语音输入后的人工修正生成待确认热词，并可选用 LLM 辅助筛选
...
- **注入开关**: 可单独控制是否向支持热词的识别引擎提供热词，不影响识别后的近音替换开关
...
| 注入热词 | 识别后替换 | 实际效果 |
|----------|------------|----------|
| 开 | 关 | 热词按供应商能力参与识别；识别后不做近音替换 |
| 开 | 开 | 热词参与识别，识别后再由目标词与别名进行近音兜底替换 |
| 关 | 开 | 不向识别引擎提供热词，但识别后仍执行近音替换 |
| 关 | 关 | 识别前后均不应用热词 |
...
1. 打开说点啥 Pro 键盘面板
2. 键盘面板上点击畅说模式开关（需要在 `设置 → 界面设置 → 自定义键盘布局` 中添加畅说模式按钮）
3. 通过按钮激活畅说模式，开始说话
4. 配置 VAD 参数:
