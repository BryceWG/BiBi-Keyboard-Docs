...
- **WebDAV 自动备份**: 定时将配置备份到云端，多设备同步（见 [配置备份](/advanced/backup-restore)）
...
在主键盘中使用 AI 后处理时，可把光标附近的输入框文本作为参考发送给 LLM，帮助模型理解上下文衔接、术语和语气。启用「输入法 Hook 模块」后，悬浮球录音也可使用当前输入框上下文辅助 AI 后处理。模块安装与隐私边界见 [IME Bridge 模块](/advanced/ime-bridge#pro-输入框上下文辅助)。
...
配置详情与故障排查见 [配置备份](/advanced/backup-restore)。

## 如何获取 Pro 版

Pro 版应用统一通过 **Google Play 商店** 分发，购买与激活方式见 [Pro 版购买方式](/pro/activation)，功能差异见 [Pro 版与开源版对比](/pro/comparison)。
