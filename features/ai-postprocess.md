...
::: info 名称说明
自 v4.4.2 起，应用内的设置入口为 `设置 → AI 功能设置`，界面文案中也将「AI 后处理」统一称为「AI 润色」。两者指同一功能。
:::
...
当 LLM 供应商支持流式输出时，AI 后处理会在处理过程中**逐步显示结果预览**（所见即所得）。您可以在 `设置 → AI 功能设置` 中开启/关闭「打字机效果」，让预览输出更平滑。
...
AI 编辑面板使用独立的系统提示词来理解“按指令修改当前文本”的任务。可在 `设置 → AI 功能设置 → AI 编辑系统提示词` 中自定义；留空时使用应用内默认提示词。
...
| `reasoningCharThreshold` | Int    | `300`  | 深度思考字数阈值（0=永久启用，300=不启用） |
| `aiEditSystemPrompt`     | String | 默认值 | AI 编辑面板使用的系统提示词，留空使用默认 |
...
对支持「推理模式」的供应商，可以在 `设置 → AI 功能设置` 中通过「深度思考字数阈值」滑块，按**本次识别文本的字数**决定是否让模型进行深度思考：
...
在 `设置 → AI 功能设置` 中，您可以通过「获取模型列表」从供应商拉取可用模型，并将常用模型添加到应用内的下拉选择中。
